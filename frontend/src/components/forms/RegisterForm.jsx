...
import { Mail, Lock, User, Hash, BookOpen, UserPlus, AlertCircle, Info } from 'lucide-react';
 ...
 * Teacher fields: full_name, email, password  (no department — admin assigns via Admin Panel)
 */
export default function RegisterForm() {
  ...
  // Fetch departments from backend (public endpoint) — only needed for students
  ...
  const validate = () => {
    ...
    // Teachers do NOT select a department on registration — admin assigns it
    return e;
  };
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-4">
      ...
      <Input id="reg-password" label="Password"       type="password" placeholder="Min 8 characters"   value={form.password     ?? ''} onChange={set('password')}     error={errors.password}     leftIcon={<Lock className="w-4 h-4" />} />
        ...
        {role === 'teacher' && (
          /* Info notice — department is assigned by admin after registration */
          <div className="flex items-start gap-3 bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-3">
            <Info className="w-4 h-4 text-indigo-500 mt-0.5 shrink-0" />
            <div>
              <p className="text-xs font-semibold text-indigo-800">Department assigned by Admin</p>
              <p className="text-xs text-indigo-600 mt-0.5">
                Your department(s) and course assignments will be configured by the system administrator after your account is created.
              </p>
            </div>
          </div>
        )}
